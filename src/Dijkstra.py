...
def fun_make_grid():
	...
	while(q1.qsize()):
		...
		if(ver_y+1<n):	
			if(distance[ver_y][ver_x] +grid_val_down[ver_y][ver_x]<distance[ver_y+1][ver_x]):
				...
				q1.put((distance[ver_y+1][ver_x],(ver_x,ver_y+1)))
		if(ver_y-1>=0):
		...
		if(ver_x+1<m):		
			if(distance[ver_y][ver_x] +grid_val_right[ver_y][ver_x]<distance[ver_y][ver_x+1]):
				...
				q1.put((distance[ver_y][ver_x+1],(ver_x+1,ver_y)))
		if(ver_x-1>=0):		
	...
	while(len(q2)):
		...
		if(ver_x+1<m and red_arr.count((ver_x,ver_y))>0):
			if(min_dis>distance[ver_y][ver_x+1]):
				min_dis=distance[ver_y][ver_x+1]
				min_ind=(ver_x+1,ver_y)
